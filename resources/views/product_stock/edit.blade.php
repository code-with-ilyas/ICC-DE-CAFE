...
<div class="d-flex justify-content-center mt-5">
    <div class="card shadow-sm p-4" style="width: 600px;">
        <h3 class="mb-4 text-white">Edit Product Stock</h3>
        ...
        <form action="{{ route('product-stock.update', $productStock->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="product_id" id="product_id" class="form-select" required>
                    ...
                    <option value="{{ $product->product_id }}" {{ $productStock->product_id == $product->product_id ? 'selected' : '' }}>{{ $product->name }}</option>
                    @endforeach
                </select>
                @error('product_id')
            ...
            <div class="mb-3">
                ...
                <select name="stock_id" id="stock_id" class="form-select" required>
                    ...
                    <option value="{{ $stock->id }}" {{ $productStock->stock_id == $stock->id ? 'selected' : '' }}>{{ $stock->name }} ({{ $stock->unit }})</option>
                    @endforeach
                </select>
                @error('stock_id')
                <div class="mt-1 text-danger" style="font-size: 0.875em;">{{ $message }}</div>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" name="quantity" id="quantity" class="form-control" value="{{ $productStock->quantity }}" required>
                @error('quantity')
                <div class="mt-1 text-danger" style="font-size: 0.875em;">{{ $message }}</div>
                @enderror
            </div>
